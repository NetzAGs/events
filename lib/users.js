...
if(Meteor.isServer) {
	Meteor.publish('users', function() {
		if(
				...
				|| Roles.getGroupsForUser(this.userId, 'event_admin').length>0
				|| Roles.getGroupsForUser(this.userId, 'task_admin').length>0
				|| Roles.getGroupsForUser(this.userId, 'coordinator').length>0
		  ) {
...
Meteor.methods({
	...
	'users.create'() {
		if(Meteor.isServer) {
			if(Roles.userIsInRole(this.userId, 'system_admin') || Roles.getGroupsForUser(this.userId, 'coordinator').length>0) { 
	...
	'users.update'(uid, updatedUser) {
		if(Meteor.isServer) {
			if(this.userId == uid || Roles.userIsInRole(this.userId, 'system_admin') || Roles.getGroupsForUser(this.userId, 'coordinator').length>0) {
				if(!Meteor.users.findOne({_id: uid})) {

					const temp = uid;
                    let existing = Accounts.findUserByEmail(updatedUser.email);
                    if(existing) {
                        uid = existing._id;
